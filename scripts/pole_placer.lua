...
--- @param polite boolean|nil Polite mode flag
--- @param drill_spacing number|nil Actual center-to-center drill spacing (if nil, uses body size)
--- @return number placed
--- @return number skipped
function pole_placer.place_substations_efficient(surface, force, player, belt_lines, drill_info, pole_info, quality, belt_direction, inter_pair_centers, polite, drill_spacing)
    local placed = 0
    local skipped = 0
    ...
    local spacing = drill_spacing or (orientation == "NS" and drill_info.height or drill_info.width)

    local positions_cache = {}

    for center_idx, cross_pos in ipairs(inter_pair_centers) do
        -- Collect unique along-positions from both adjacent belt lines for this gap.
        -- Adjacent pairs often share the same drill positions, so dedup to avoid
        -- placing duplicate ghost entities at the same coordinates.
        local along_set = {}
        local adjacent_indices = {center_idx, center_idx + 1}
        for _, belt_idx in ipairs(adjacent_indices) do
            local belt_line = belt_lines[belt_idx]
            if belt_line then
                local drill_positions = belt_line.drill_along_positions or {}
                if #drill_positions > 0 then
                    local count = #drill_positions
                    if not positions_cache[count] then
                        positions_cache[count] = pole_placer.calculate_positions(pole_info, count, spacing, belt_direction)
                    end
                    local positions_set = positions_cache[count]

                    for i, along_pos in ipairs(drill_positions) do
                        if positions_set[i] then
                            along_set[math.floor(along_pos)] = true
                        end
                    end
                end
            end
        ...
        for along_pos, _ in pairs(along_set) do
            local pos
            if orientation == "NS" then
                pos = {x = cross_pos, y = along_pos}
            else
                pos = {x = along_pos, y = cross_pos}
            end
